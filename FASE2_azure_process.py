...
class AzurePronunciationEvaluator:
    ...
    def _create_recognizer(self):
        self._done_event = threading.Event()
        self.stream = speechsdk.audio.PushAudioInputStream(self.audio_format)
    ...
    def _on_interim(self, evt):
        console.print(
            f"[yellow][Azure Pron interim][/yellow] {evt.result.text}", end="\r"
        )
    ...
    def _on_final(self, evt):
            ...
            self.results["azure_pronunciation"]["word_timings"].append(
                {
                    ...
                    "accuracy_score": round(
                        w.get("PronunciationAssessment", {}).get("AccuracyScore", 0.0),
                        1,
                    ),
        ...
        self.results["azure_pronunciation"]["pronunciation_scores"] = {
            ...
            "prosody_score": (
                round(pa.get("ProsodyScore", 0.0), 1) if "ProsodyScore" in pa else None
            ),
        }
    ...
    def _on_interim(self, evt):
        ...
        if self.results is not None:
            self.results["azure_plain"]["interim_transcripts"].append(
                {"text": txt, "timestamp": datetime.now(timezone.utc).isoformat()}
            )
