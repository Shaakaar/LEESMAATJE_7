      ...
      units: [
        {
          ...
          strict_forbid: false,
          focus_klanken: [],
          allowed_patterns: ['CV', 'CVC', 'CVCC', 'CCVC', 'CVCV', 'CVCCV', 'CVCVC'],
          word_bank: [],
          sentence_rules: {
            present: true,
            punctuation: 'period_only',
...
export function findLevelUnit(levelId: string, unitId: string) {
...
export function getAllowedKlanken(level: LevelSpec, upToUnitId: UnitId): string[] {
  const idx = level.units.findIndex((u) => u.id === upToUnitId);
  if (idx === -1) return [];
  return Array.from(new Set(level.units.slice(0, idx + 1).flatMap((u) => u.focus_klanken)));
}
